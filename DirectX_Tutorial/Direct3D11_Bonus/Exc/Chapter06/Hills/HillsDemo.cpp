...
HRESULT CALLBACK OnD3D11CreateDevice(ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
    ...
    V_RETURN(DXUTCompileFromFile(L"D:/Work/Direct3D11/Exc/Chapter06/Hills/HillsDemo.fx", nullptr, "VS", "vs_5_0", dwShaderFlags, 0, &pVSBlob));
    ...
    ID3D10Blob* pPSBolob = nullptr;
    V_RETURN(DXUTCompileFromFile(L"D:/Work/Direct3D11/Exc/Chapter06/Hills/HillsDemo.fx", nullptr, "PS", "ps_5_0", dwShaderFlags, 0, &pPSBolob));

    // Create the pixel shader
    hr = pd3dDevice->CreatePixelShader(pPSBolob->GetBufferPointer(), pPSBolob->GetBufferSize(), nullptr, &g_pPixelShader);
    SAFE_RELEASE(pPSBolob);
    if (FAILED(hr))
        return hr;


    // Build Geometry Buffers (GeometryGenerator)
    ...
    for (size_t i = 0; i < grid.Vertices.size(); i++)
    {
        ...
        p.y = 0.3f * (p.z * sinf(0.1 * p.x) + p.x * cosf(0.1f * p.z));

        vertices[i].Pos = p;
        if (p.y < -10.0f)
        {
            // Sandy beach color.
            vertices[i].Color = XMFLOAT4(1.0f, 0.96f, 0.7f, 0.5f);
        }
        else if (p.y < 5.0f)
        {
            // Light yellow-green.
    ...
    bd.ByteWidth = sizeof(SimpleVertex) * grid.Vertices.size();
    ...
    V_RETURN(pd3dDevice->CreateBuffer(&bd, &vInitData, &g_pVertexBuffer));
    ...
    bd.ByteWidth = sizeof(UINT) * mGridIndexCount;
    bd.Usage = D3D11_USAGE_IMMUTABLE;
    ...
    V_RETURN(pd3dDevice->CreateBuffer(&bd, &iInitData, &g_pIndexBuffer));
    ...
    // Initialize the view matrix - Convert Spherical to Cartesian coordinates.
    float x = 200.0f * sinf(0.1f * DirectX::XM_PI) * cosf(1.5f * DirectX::XM_PI);
    float z = 200.0f * sinf(0.1f * DirectX::XM_PI) * sinf(1.5f * DirectX::XM_PI);
    float y = 200.0f * cosf(0.1f * DirectX::XM_PI);
    static const XMVECTORF32 s_Eye = { 0.0f, 300.0f, -500.0f, 0.f };
...
void CALLBACK OnFrameMove(double fTime, float fElapsedTime, void* pUserContext)
{
    ...
    g_World = XMMatrixRotationY(45.0f * XMConvertToRadians((float)fTime));
    // g_World = XMMatrixIdentity();
}
    ...
    double fTime, float fElapsedTime, void* pUserContext)
{
    ...
    pd3dImmediateContext->DrawIndexed(mGridIndexCount, 0, 0);
}
