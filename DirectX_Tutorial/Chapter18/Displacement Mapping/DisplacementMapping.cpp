...
void DrawScene(ID3D11DeviceContext* pd3dImmediateContext, CModelViewerCamera camera, bool drawCenterSphere)
{
	...
	Effects::DisplacementMapFX->SetDirLights(g_DirectionalLights);
	Effects::DisplacementMapFX->SetCubeMap(g_Sky->GetSkyCubeMap());
	...
	ID3DX11EffectTechnique* activeTexTech = Effects::DisplacementMapFX->Light3TexTech;
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::DisplacementMapFX->SetNormalMap(mFloorNormalTexSRV);
	...
	double fTime, float fElapsedTime, void* pUserContext)
{
	...
	pd3dImmediateContext->HSSetShader(0, 0, 0);
	pd3dImmediateContext->DSSetShader(0, 0, 0);
