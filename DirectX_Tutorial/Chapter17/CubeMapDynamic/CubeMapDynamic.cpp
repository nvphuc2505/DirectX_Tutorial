	...
	double fTime, float fElapsedTime, void* pUserContext)
{
	...
	pd3dImmediateContext->RSSetViewports(1, &g_CubeMapViewport);
	for (int i = 0; i < 6; ++i)
	{
		// Clear cube map face and depth buffer.
		pd3dImmediateContext->ClearRenderTargetView(g_DynamicCubeMapRTV[i], reinterpret_cast<const float*>(&Colors::Silver));
		pd3dImmediateContext->ClearDepthStencilView(g_DynamicCubeMapDSV, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);

		// Bind cube map face as render target.
		renderTargets[0] = g_DynamicCubeMapRTV[i];
		pd3dImmediateContext->OMSetRenderTargets(1, renderTargets, g_DynamicCubeMapDSV);

		// Draw the scene with the exception of the center sphere to this cube map face.
		DrawScene(pd3dImmediateContext, g_CubeMapCamera[i], false);
	}
	...
	renderTargets[0] = pRTV;
	pd3dImmediateContext->OMSetRenderTargets(1, renderTargets, pDSV);

	// Generate mipmaps for the cube map
	pd3dImmediateContext->GenerateMips(g_DynamicCubeMapSRV);

	// Now draw the scene normally, but include the center sphere
	...
	DrawScene(pd3dImmediateContext, g_Camera, true);
}
