...
float AnimationClip::GetClipStartTime() const
{
	...
	for (BoneAnimation boneAnim : BoneAnimations)
		t = MathHelper::Min(t, boneAnim.GetStartTime());
...
float AnimationClip::GetClipEndTime() const
{
	float t = MathHelper::Infinity;
	for (BoneAnimation boneAnim : BoneAnimations)
		t = MathHelper::Max(t, boneAnim.GetEndTime());
...
void AnimationClip::Interpolate(float t, std::vector<XMFLOAT4X4>& boneTransforms) const
{
	...
	{
		// Interpolate each bone's transformation at time t
		BoneAnimations[i].Interpolate(t, boneTransforms[i]);
	}
}


bool SkinnedData::HasAnimations() const
{
...
void SkinnedData::GetFinalTransforms(const std::string& clipName, float timePos, std::vector<XMFLOAT4X4>& finalTransforms) const
{
	...
	// Ensure the finalTransforms vector is resized appropriately
	...
	{
		// Handle error: Animation clip not found
		return;
	}
	...
	// Root bone has no parent, so it's just the local bone transform
